...
### Запуск доп. части (связь с другими ЯП)

Устанавливаем poetry:

**`pipx install poetry`**

Создаем виртуальную среду и загружаем зависимости:

**`poetry install`**

Теперь код готов к запуску

**Используем только "insert.py" и "statistic.py" файлы!!!!**

Файл insert.py вставит нужные данные в базу, но первоначально нужно указать все необходимые данные для подключения к бд **`URL="postgresql+psycopg2://{user}:{password}@{host}/{database}"`** 
(перед защитой сделаю нормальный .env)

Затем запустить файл **`insert.py`**, автоматически создастся схема и заполнится база данных

Теперь можно запустить файл **`statistic.py`**. После запуска все графики будут выводиться в браузере при помощи фрэймворка для работы с данными Dash. В терминале будет ссылка, либо можно сразу зайти по локальному адресу http://127.0.0.1:8050/.